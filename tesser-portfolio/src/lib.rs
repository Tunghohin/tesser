...
impl Portfolio {
    ...
    /// Update positions in response to an execution fill and return realized PnL.
    pub fn apply_fill_positions(&mut self, fill: &Fill) -> PortfolioResult<FillLedgerImpact> {
        let instrument = self
            .market_registry
            .get(fill.symbol)
    ...
    /// Apply externally generated ledger entries to the cash book.
    pub fn apply_ledger_entries(&mut self, entries: &[LedgerEntry]) -> PortfolioResult<()> {
        let reporting = self.reporting_currency;
        for entry in entries {
            let account = self.account_mut(entry.exchange);
            account.ensure_currency(reporting, entry.asset);
            account.balances.adjust(entry.asset, entry.amount);
        }
        self.update_drawdown_state();
        Ok(())
    }
...
mod tests {
    ...
    fn portfolio_updates_equity() {
        ...
        apply_with_ledger(&mut portfolio, &buy);
    ...
    fn triggers_liquidate_only_on_drawdown() {
        ...
        let mut portfolio = Portfolio::new(config, registry.clone());
    ...
    fn apply_with_ledger(portfolio: &mut Portfolio, fill: &Fill) {
        let impact = portfolio.apply_fill_positions(fill).unwrap();
        let registry = sample_registry();
        let instrument = registry.get(fill.symbol).unwrap();
        let entries = entries_from_fill(FillLedgerContext::new(
            fill,
            &instrument,
            impact.realized_pnl,
        ));
        portfolio.apply_ledger_entries(&entries).unwrap();
    }
}
